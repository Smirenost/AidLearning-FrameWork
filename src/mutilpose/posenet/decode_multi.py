...
def get_instance_score(
    existing_poses, squared_nms_radius, keypoint_scores, keypoint_coords
):
...
def score_is_max_in_local_window(
    keypoint_id, score, hmy, hmx, local_max_radius, scores
):
        ...
        for hmx in range(width):
            for keypoint_id in range(num_keypoints):
                ...
                if score < score_threshold:
                    continue
                if score_is_max_in_local_window(
                    keypoint_id, score, hmy, hmx, local_max_radius, scores
                ):
...
def decode_multiple_poses(
    ...
    for root_score, root_id, root_coord in scored_parts:
        root_image_coords = (
            root_coord * output_stride + offsets[root_coord[0], root_coord[1], root_id]
        )
